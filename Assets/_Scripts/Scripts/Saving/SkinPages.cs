...
public class SkinPages
{
    ...
    public SkinPages(SkinPages from)
    {
        ...
        SkinListForReference = new List<SkinList>();
        foreach (var VARIABLE in from.SkinListForReference)
        {
            var newSkinList = new SkinList(VARIABLE);
            SkinListForReference.Add(newSkinList);
        }
    }
